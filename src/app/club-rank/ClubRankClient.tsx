...
const normalizeType = (val?: string) => {
  ...
  if (["±¹Á¦", "±¹Á¦´ëÈ¸", "international", "intl", "inter"].includes(v)) return "±¹Á¦´ëÈ¸";
  if (["±¹³»", "±¹³»´ëÈ¸", "domestic", "local"].includes(v)) return "±¹³»´ëÈ¸";
  return val ?? "";
};

const normalizeRank = (val?: string) => {
  const v = normalize(val);
  if (["winner", "win", "champ", "¿ì½Â", "1", "1À§", "first"].includes(v)) return "winner";
  if (["runnerup", "runner-up", "ru", "ÁØ¿ì½Â", "2", "2À§", "second"].includes(v)) return "runner-up";
  if (["third", "3", "3À§", "bronze", "µ¿¸Ş´Ş"].includes(v)) return "third";
  if (["participation", "Âü°¡", "Âü¿©"].includes(v)) return "participation";
  return val ?? "";
};

const normalizeLeagueType = (val?: string) => {
  const v = normalize(val);
  if (["open", "¿ÀÇÂ", "¿ÀÇÂºÎ", "openºÎ"].includes(v)) return "open";
  if (["2", "2ºÎ", "2ºÎ¸®±×", "division2", "d2", "div2", "2nd"].includes(v)) return "division2";
  return val ?? "";
};
...
const normalizeOtherClub = (val?: string) => {
  ...
  if (["¾Æ´Ï¿À", "¾Æ´Ï¿ä", "no", "n", "false", "0", "off"].includes(v)) return false;
  return false;
};
...
function computeMatchPoints(record: MatchRecord, rules: PointRules) {
  ...
  let base = 0;
  if (typeKey === "±¹³»´ëÈ¸" && leagueKey === "division2") {
